  ...
  if (bPlaying && nSeqNum<SEQNUM)
  {
    for (int i=nSeqNum;i<SEQNUM;i++)
    {
      ...
      else 
      {
        ...
        curValue = LightStatus[lightSeq[i].channel] + (int)((lightSeq[i].value - LightStatus[lightSeq[i].channel]) * (now - lightSeq[i].startTime) / (lightSeq[i].stopTime - lightSeq[i].startTime));
      }
