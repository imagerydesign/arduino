  ...
  if (bPlaying && nSeqNum<SEQNUM)
  {
    for (int i=nSeqNum;i<SEQNUM;i++)
    {
      ...
      else 
      {
        ...
        curValue = LightStatus[lightSeq[i].channel] + (int)(((double)(lightSeq[i].value - LightStatus[lightSeq[i].channel])) * ((double)(now - realLastTime)) / ((double)(lightSeq[i].stopTime - realLastTime)));
      }
      DmxSimple.write (lightSeq[i].channel, curValue);
      Serial.println(realLastTime + " : "+now); 
      Serial.println(LightStatus[lightSeq[i].channel] + ":" + curValue);
