  ...
  if (bPlaying && nSeqNum<SEQNUM)
  {
    for (int i=nSeqNum;i<SEQNUM;i++)
    {
      ...
      else 
      {
        ...
        curValue = LightStatus[lightSeq[i].channel] + (int)(((double)(now - lightSeq[i].startTime)) / ((double)(lightSeq[i].stopTime - lightSeq[i].startTime)) * ((double)(lightSeq[i].value - LightStatus[lightSeq[i].channel])));
      }
